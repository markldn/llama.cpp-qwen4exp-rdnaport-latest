...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    // expert gets touched; hparams.n_expert_used(il) gives the real per-layer count to bound
    // the graph to instead (ref: https://github.com/ggml-org/llama.cpp/pull/14753). Outside
    // warmup, n_expert_used is already the real count that selected_experts/weights were built
    // with above -- possibly overridden by --n-expert-used-prefill or --n-expert-used-decode
    // depending on the ubatch kind -- so use it directly rather than re-deriving
    // hparams.n_expert_used(il), which would mismatch under either override. Upstream's own
    // per-layer n_expert_used(il) call (added alongside the Puzzle-arch fix this merge pulled
    // in) is adopted for the warmup branch's per-layer correctness, but the override-aware
    // parameter is kept for the non-warmup path since hparams alone doesn't know about it.
    const uint32_t n_expert_used_agg = cparams.warmup ? hparams.n_expert_used(il) : (uint32_t) n_expert_used;
