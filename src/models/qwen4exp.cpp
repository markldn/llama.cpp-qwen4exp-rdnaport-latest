...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // so the early output masking has to be skipped (it is applied after the final mix instead)
...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    // upstream is generalizing native sparse-FA into build_attn_mha itself now (extra
    // top-k arg, ref: https://github.com/ggml-org/llama.cpp/pull/27970) but has it
    // disabled (0 = dense) pending readiness. Not adopted here - this fork already has
    // its own validated sparse path above (LLAMA_QSA_SPARSE_FA); this call is just the
    // dense fallback, adapted to upstream's new signature with sparse left off (0),
    // same semantics as before the merge.
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn(
    ...
    // mctx_hyb is null for the MTP draft block, which always attends dense (no QSA cache of its own).
