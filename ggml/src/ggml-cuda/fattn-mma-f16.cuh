...
static constexpr __host__ __device__ fattn_mma_config ggml_cuda_fattn_mma_get_config_rdna(const int DKQ, const int DV, const int ncols) {
    ...
    // RDNA4 perf tune (rdna-boosts block 04), not touched by upstream's independent row-32
    // change above - keep our measured config for this row.
...
static __global__ void flash_attn_ext_f16(
    ...
    // matches the RDNA4/RDNA3.5 head-dim cap in the fattn.cu dispatch heuristic above -
    // must stay consistent with wmma_max_head there or dispatch could route a head size
    // this device code then refuses.
